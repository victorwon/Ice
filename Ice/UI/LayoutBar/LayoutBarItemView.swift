...
final class LayoutBarItemView: NSView {
    ...
    func provideAlertForDisabledItem() -> NSAlert {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("Menu bar item is not movable.", comment: "")
        alert.informativeText = String(format: NSLocalizedString("macOS prohibits \"%@\" from being moved.", comment: ""), item.displayName)
        return alert
    }
    ...
    func provideAlertForUnresponsiveItem() -> NSAlert {
        ...
        alert.informativeText = String(format: NSLocalizedString("%@ is unresponsive. Until it is restarted, it cannot be moved. Movement of other menu bar items may also be affected until this is resolved.", comment: ""), item.displayName)
        return alert
    }
